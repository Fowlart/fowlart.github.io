...
public class OnServerFileCreator {
    ...
    private String HTML_FILE = "player.html";
    ...
    public void createHtmlPage() {
        ...
        if (!html.exists()) {
            try {
                html.createNewFile();
                System.out.println(">created: " + html.getAbsolutePath());
                FileWriter fileWriter = new FileWriter(html);
                fileWriter.append(HTML_CODE);
                fileWriter.flush();
                fileWriter.close();
                System.out.println(">html page was created " + html.getAbsolutePath());
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
